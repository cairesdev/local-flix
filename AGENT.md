...
- **EmbedTV** (`src/services/embedtv.ts`): resolve a URL do player de um canal (`getEmbedPlayerUrl(channel, useProxy)`) usando a URL própria do canal (`channel.url`, retornada pelo `channels_url` do provedor - cada canal pode estar em um sub-domínio diferente, ex. `ww1`/`ww4`/`ww5`), com fallback para `player_base_url` + id quando o provedor não informa URL por canal.
